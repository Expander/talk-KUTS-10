...
\begin{frame}{EFT calculation in \fs (\texttt{HSSUSY})}
  ...
  \begin{tikzpicture}
    ...
    \draw[thick,darkgreen] (0,4) node[left]{$\Qmatch = M_S$} -- node[above = 0.5cm,black]{MSSM} (3,4) node[right,black,text width=8cm]{calculate $\lambda$};
    \draw[thick,dashed,darkgreen] (0,2) node[left]{$\Qpole = M_t$} -- (3,2) node[right,black,text width=8cm]{calculate $M_h(\lambda, v, g_i, y_f)$};
    \draw[thick,darkgreen] (0,1) node[left]{$\Qlow = M_Z$} -- node[above = 1.5cm,black]{SM} node[below = 0.5cm,black]{SM(5)} (3,1) node[right,black,text width=8cm]{
      calculate $y_f$, $g_i$, $v$};
    \path[arrow,latex-latex,blue] (-2.5,1) -- node[above,rotate=90]{RG running} (-2.5,4);
  \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{Intermediate conclusions for $M_h$}
  ...
  \item 3-loop $O(\as^3)$ corrections to $m_t$: $\DMhHSSUSYytSM \lesssim 400\MeV$
  \item 4-loop $O(\as^4)$ corrections to $m_t$: $\Delta M_h \lesssim 125\MeV$
