...
\begin{frame}{Uncertainty estimate of the fixed-order calculation}
  ...
  \begin{align*}
    ...
    \DMhQmatch &= \max_{\Qmatch\in[M_Z/2,2M_Z]}\left|M_h(\Qmatch) - M_h(M_Z)\right| \\
    \DMhMt &= \left|M_h(m_t^{(1)}) - M_h(m_t^{(2)})\right| \\
    \DMhAlphaS &= \left| M_h(\as^{(1)}) - M_h(\as^{(2)}) \right|  \\
    \DMhAlphaEm &= \left| M_h(\aem^{(1)}) - M_h(\aem^{(2)}) \right|
  \end{align*}
  Combination:
  \begin{align*}
...
\begin{frame}{Uncertainty estimate of the fixed-order calculation}
  Inspired by \bigcite{1407.4081,1504.05200}, we consider 5 sources of
  uncertainty:
  \begin{align*}
    ...
    \DMhHSSUSYytSM &= \left| M_h(y_t^{\SM,2L}(M_Z)) - M_h(y_t^{\SM,3L}(M_Z)) \right| \\
    \DMhEFT &= \left| M_h - M_h(v^2/\MS^2) \right| \\
    \DMhHSSUSYytMSSM &= \left| M_h - M_h(y_t^\MSSM(\MS)) \right|
  \end{align*}
  Combination:
  \begin{align*}
