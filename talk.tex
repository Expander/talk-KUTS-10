...
\begin{frame}{EFT calculation in \fs (\texttt{HSSUSY})}
  ...
  \begin{tikzpicture}
    ...
    \draw[thick,darkgreen] (0,4) node[left]{$M_S$} -- node[above = 0.5cm,black]{MSSM} (3,4) node[right,black,text width=8cm]{
      $\lambda$ at $O(\text{1L} + (\at+\ab)\as + (\at+\ab)^2)$\\ \ \ \ \ \ + $O(\ab\atau + \atau^2)$ + $O(\at\as^2)$};
    \draw[thick,dashed,darkgreen] (0,2) node[left]{$M_t$} -- (3,2) node[right,black,text width=8cm]{
      $M_h^2$ at $O(\text{1L} + (\at+\ab)\as + (\at + \ab)^2)$\\ \ \ \ \ \ \ + $O(\at^3 + \at^2\as + \at\as^2)$ + $O(\at\as^3)$
    };
    ...
    \path[arrow,latex-latex,blue] (-1,1) -- node[above,rotate=90]{5L RGEs} (-1,4);
  \end{tikzpicture}
  \end{center}
